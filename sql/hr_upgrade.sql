...
-- 注意：不能用 SELECT * FROM (SELECT 常量...) AS t —— MySQL 8.0 会因派生表列名重复
--      （两个 NULL 都叫 'NULL'）报 ERROR 1060 Duplicate column name 'NULL'，故改用 FROM DUAL。
INSERT INTO `leave` (`leave_id`,`emp_id`,`reason`,`start_date`,`end_date`,`state`,`apply_date`,`apply_time`,`approve_date`,`approve_time`)
SELECT 1,2,'老家有事，需回家处理','2026-09-15','2026-09-17',0,'2026-09-11','09:12:00',NULL,NULL
FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM `leave` LIMIT 1);
INSERT INTO `leave` (`leave_id`,`emp_id`,`reason`,`start_date`,`end_date`,`state`,`apply_date`,`apply_time`,`approve_date`,`approve_time`)
SELECT 2,4,'身体不适，医院就诊','2026-09-18','2026-09-19',0,'2026-09-11','14:30:00',NULL,NULL
FROM DUAL WHERE (SELECT COUNT(*) FROM `leave`) = 1;
INSERT INTO `leave` (`leave_id`,`emp_id`,`reason`,`start_date`,`end_date`,`state`,`apply_date`,`apply_time`,`approve_date`,`approve_time`)
SELECT 3,5,'病假休养（已同意，未销假，处于请假状态）','2026-09-08','2026-09-14',1,'2026-09-05','10:20:00','2026-09-06','09:00:00'
FROM DUAL WHERE (SELECT COUNT(*) FROM `leave`) = 2;
INSERT INTO `leave` (`leave_id`,`emp_id`,`reason`,`start_date`,`end_date`,`state`,`apply_date`,`apply_time`,`approve_date`,`approve_time`)
SELECT 4,7,'婚假（已销假）','2026-07-01','2026-07-05',3,'2026-06-28','08:40:00','2026-07-06','08:30:00'
FROM DUAL WHERE (SELECT COUNT(*) FROM `leave`) = 3;
INSERT INTO `leave` (`leave_id`,`emp_id`,`reason`,`start_date`,`end_date`,`state`,`apply_date`,`apply_time`,`approve_date`,`approve_time`)
SELECT 5,8,'个人事务请假（已驳回）','2026-06-10','2026-06-11',2,'2026-06-08','16:05:00','2026-06-09','10:30:00'
FROM DUAL WHERE (SELECT COUNT(*) FROM `leave`) = 4;
-- 让自增主键继续
-- 注意：MySQL 的 ALTER TABLE ... AUTO_INCREMENT 不接受变量/表达式（会报 1064），
--      必须拼成 SQL 字符串后用预处理语句执行。
SET @m = (SELECT IFNULL(MAX(leave_id), 0) FROM `leave`);
SET @s = CONCAT('ALTER TABLE `leave` AUTO_INCREMENT = ', @m + 1);
PREPARE stmt FROM @s;
EXECUTE stmt;
