...
- **决策依据板块（新增）**：
  · 网格员人力与增员需求 `GET /api/stats/workforce`：总数/在岗/忙碌/空闲/请假中，
    是否需要增员与建议增员人数（依据见 needReasons）、缺员区域、各区域在岗-忙碌对比。
    口径：忙碌 = 在岗且名下有未完成指派任务；人力缺口 = 待指派任务数超出
    「空闲网格员 × 人均在办上限 `nep.task.worker-capacity`（默认 3）」
  · 反馈覆盖度环比 `GET /api/stats/feedbackCoverage`：本月 vs 上月各城市反馈量与环比、
    反馈较多/较少榜单，并给出每个城市“多/少”的结论——重点区分**环境良好**
    （有实测样本且平均等级 ≤2）与**覆盖不足**（该区域无在岗网格员，反馈少更可能是
    缺少检测覆盖而非环境好），以及**反馈减少/增加**（公众参与度变化）
  · **决策者可提交增援申请**：大屏“待增援区域”列出「有任务却无在岗网格员」的区域
    （`workforce.needWorkerRegions`），未提交过的区域可直接点「提交增援申请」。
    大屏看不到具体反馈，因此按区域提交（`provinceId/cityId` + `source=viewer`），
    后端自动关联该区域最早的待指派反馈以便追溯；已提交的显示“已提交待处理 #编号”。
    管理员在“指派”弹窗里按反馈发起的入口同时保留
  · AI 治理意见（结合污染原因给出处置建议）为后续迭代项，本轮未实现
